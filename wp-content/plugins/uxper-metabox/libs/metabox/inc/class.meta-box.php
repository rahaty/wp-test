...
if ( is_admin() ) {
	add_action( 'load-post.php', 'uxper_framework_register_meta_boxes' );
	add_action( 'load-post-new.php', 'uxper_framework_register_meta_boxes' );
}

function uxper_framework_register_meta_boxes() {

	$meta_boxes = apply_filters( 'uxper_metabox_meta_boxes', array() );
	...
	foreach ( $meta_boxes as $meta_box ) {
		...
		if ( in_array( $screen->id, $meta_box['post_types'] ) && $screen->base == 'post' ) {
			new Uxper_Framework_Meta_Box( $meta_box );
		}
	}
}

if ( ! class_exists( 'Uxper_Framework_Meta_Box' ) ) {
	class Uxper_Framework_Meta_Box {
		...
		function __construct( $meta_box ) {
			...
			add_action( 'add_meta_boxes', array( $this, 'uxper_add_meta_boxes' ) );
			...
			foreach ( $this->meta_box['post_types'] as $post_type ) {
				if ( 'attachment' === $post_type ) {
					...
					add_action( 'edit_attachment', array( $this, 'uxper_meta_box_save' ) );
					add_action( 'add_attachment', array( $this, 'uxper_meta_box_save' ) );
				} else {
					add_action( "save_post_{$post_type}", array( $this, 'uxper_meta_box_save' ) );
				}
			}
		}

		function enqueue_scripts() {
			$fields = Uxper_Framework_Helper::get_fields( $this->fields );

			foreach ( $fields as $field ) {
				$class = 'UXPER_' . $field . '_Field';
		...
		function uxper_add_meta_boxes() {
			foreach ( $this->meta_box['post_types'] as $post_type ) {
				add_meta_box(
					$this->meta_box['id'],
					$this->meta_box['title'],
					array( $this, 'uxper_meta_box_output' ),
					$post_type,
		...
		function uxper_meta_box_output( $post ) {
			// Check whether form is submitted properly
			wp_nonce_field( "uxper-save-" . $this->id, "nonce_" . $this->id );
			...
			if ( $this->group == true ) {
				...
				if ( ! empty( $this->fields ) ) {
					Uxper_Framework_Helper::render_form( $this->fields, $post_metas );
				...
				if ( ! empty( $this->fields ) ) {
					Uxper_Framework_Helper::render_form( $this->fields, self::$meta_post_values );
				}
			}
		}

		function uxper_meta_box_save( $post_id ) {
			/*
			 * We need to verify this came from our screen and with proper authorization,
			 * because the save_post action can be triggered at other times.
			 */
			$nonce = isset( $_POST[ "nonce_" . $this->id ] ) ? sanitize_key( $_POST[ "nonce_" . $this->id ] ) : '';
			if ( empty( $_POST[ "nonce_" . $this->id ] ) || ! wp_verify_nonce( $nonce, "uxper-save-" . $this->id ) ) {
				return;
			}
			...
			if ( $this->group == true ) {
				$metas = Uxper_Framework_Helper::get_form_values( $this->fields );
			...
			foreach ( $fields as $field ) {
				if ( in_array( $field['type'], array( 'tabpanel', 'accordion' ) ) ) {
					...
					if ( isset( $field['id'] ) && isset( $_POST[ $field['id'] ] ) ) {
						$class = 'UXPER_' . $field['type'] . '_Field';
