...
class ThePlus_Off_Canvas extends Widget_Base {
    ...
    protected function _register_controls() {
		...
		$this->start_controls_section(
			'content_section',
			[
				'label' => esc_html__( 'Popup Builder Content', 'theplus' ),
		...
		$this->add_control(
			'content_template',
			[
				'label'       => esc_html__( 'Select Templates', 'theplus' ),
		...
		$this->add_control(
			'openTriggerNote',
			[
				'label' => esc_html__( 'Note : You need to Select Option Hide from Popup Builder Content Tab', 'theplus' ),
		...
		$this->add_control(
			'inactivitySec',
			[
				'label' => esc_html__( 'Inactivity MiliSeconds', 'theplus' ),
	 ...
	 protected function render() {
		...
		if($flag) {
			...
			$data_attr = 'data-settings={"content_id":"'.$content_id.'","transition":"'.esc_attr($content_open_style).'","direction":"'.esc_attr($content_open_direction).'","esc_close":"'.esc_attr($event_esc_close_content).'","body_click_close":"'.esc_attr($event_body_click_close_content).'","trigger":"' . esc_attr ( $openTrigger ) . '","tpageload":"' . esc_attr ( $pageload ) . '","tscroll":"' . esc_attr ( $scroll ) . '","scrollHeight":"'. esc_attr($scrollHeight). '","texit":"' . esc_attr ( $exit ) . '","tinactivity":"' . esc_attr ( $inactivity ) . '","tpageviews":"' . esc_attr ( $pageviews ) . '","tpageviewscount":"' . esc_attr ( $PVC ) . '","tprevurl":"' . esc_attr ( $prevurl ) . '","previousUrl":"'. esc_attr($previousUrl). '","textraclick":"' . esc_attr ( $extraclick ) . '","extraId":"'. esc_attr($extraId). '","extraIdClose":"'. esc_attr($extraIdClose). '","inactivitySec":"'. esc_attr($inactivitySec). '","sr":"'. esc_attr($sr). '","srxtime":"'. esc_attr($srxtime). '","srxdays":"'. esc_attr($srxdays). '"}';
			//button
			$toggle_content='';
			...
			$off_canvas ='<div class="plus-offcanvas-wrapper '.esc_attr($widget_uid).' '.$animated_class.' '.esc_attr($show_scroll_window_offset).'" data-canvas-id="'.esc_attr($widget_uid).'" '.$data_attr.' '.$scroll_top_offset_value.' '.$animation_attr.' '.$PVCStyle.'>';
