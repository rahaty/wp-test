 ...
 */
abstract class Sala_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		add_action( 'save_post', array( $this, 'sala_widget_cache' ) );
		add_action( 'deleted_post', array( $this, 'sala_widget_cache' ) );
		add_action( 'switch_theme', array( $this, 'sala_widget_cache' ) );
	...
	public function enqueue_resources(){
		// base
		wp_enqueue_style(SALA_THEME_PREFIX . 'widget', SALA_WIDGET_URI . '/inc/widgets/assets/css/widget.css', array(), false, 'all');
		wp_enqueue_script(SALA_THEME_PREFIX . 'widget', SALA_WIDGET_URI . '/inc/widgets/assets/js/widget.js',array(), false, true);
	}
	 ...
	 * @return void
	 */
	public function sala_widget_cache() {
			...
			switch ( $setting['type'] ) {
                ...
                case "multi-select" :
                    ...
                    <p>
                        <label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html($setting['label']); ?></label>
                        <input name="<?php echo esc_attr($this->get_field_name( $key )); ?>" type="hidden" value="<?php echo esc_attr( $value ); ?>" />
                        <select multiple="multiple" class="widefat sala-select2" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" data-value="<?php echo esc_attr($value) ?>">
                            <?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
				...
				case "icon":
					...
					<div>
						...
						<div>
							...
							<input style="float: right"  title="<?php echo esc_attr__('Click to browse icon','sala') ?>" class="browse-icon button-secondary" type="button" value="<?php echo esc_attr__('Browse...','sala') ?>" />
				...
				case "image":
					...
					<div style="margin: 13px 0">
						...
						<div class="widget-image-field">
							...
							<button style="margin-top: 10px" title="<?php echo esc_attr__('Click to browse image','sala') ?>" class="browse-image button-secondary" type="button"><?php echo esc_html__('Upload image','sala') ?></button>
						</div>
					</div>
