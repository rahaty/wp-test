 ...
 * @since emo 1.0
 */
if (!class_exists('SALA_Widget_Areas')) {
	class SALA_Widget_Areas {
		...
		public function __construct() {
			if(is_admin())
			{
				...
				add_action( 'wp_ajax_sala_delete_widget_area', array( $this, 'delete_widget_area' ) );
			}
			add_action('widgets_init',array(&$this, 'register_custom_widget_areas'),11);
		}

		public function enqueue() {
			wp_enqueue_script(SALA_PLUGIN_PREFIX.'widget-areas', plugins_url(SALA_PLUGIN_NAME . '/widgets/widget-areas/assets/js/widget-areas.min.js'), array('jquery'), $this->version);
			wp_enqueue_style(SALA_PLUGIN_PREFIX.'widget-areas', plugins_url(SALA_PLUGIN_NAME . '/widgets/widget-areas/assets/css/widget-areas.min.css'), array(), $this->version, 'screen');
			wp_localize_script(SALA_PLUGIN_PREFIX.'widget-areas',
				'sala_widget_areas_variable',
				array(
					'ajax_url' => admin_url('admin-ajax.php'),
					'confirm_delete' => esc_html__('Are you sure to delete this widget areas?', 'sala')
				)
			);
		}
		...
		public function register_custom_widget_areas() {
			...
			$args = apply_filters('sala_custom_widget_args', $args);
