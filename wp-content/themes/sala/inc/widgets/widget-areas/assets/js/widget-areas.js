(function ($) {
	...
	var SALA_Widget_Areas = {
		init : function() {
			$widgetTemplate = $('#bin-add-widget-template');
			$widgetArea = $('#widgets-right');
			$widgetWrap = $('.sidebars-column-' + $widgetArea.children().length);

			SALA_Widget_Areas.addFormHtml();
			SALA_Widget_Areas.addDelButton();
			SALA_Widget_Areas.deleteWidgetArea();
			SALA_Widget_Areas.event();
		},
		...
		deleteWidgetArea : function() {
			$widgetArea.on('click','.bin-widget-area-delete',function() {
				if (!confirm(sala_widget_areas_variable.confirm_delete)) {
					return;
				}
				...
				$.ajax({
					type: "POST",
					url: sala_widget_areas_variable.ajax_url,
					data: {
						action: 'sala_delete_widget_area',
						name: widget_name,
						_wpnonce: nonce
					},
