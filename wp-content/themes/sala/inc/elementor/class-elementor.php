...
final class Entry {
	...
	public function check_validate() {
		 ...
		 * WPML supported.
		 */
		require_once SALA_ELEMENTOR_DIR . '/wpml/class-wpml-translatable-nodes.php';

		require_once SALA_ELEMENTOR_DIR . '/class-widget-utils.php';
		require_once SALA_ELEMENTOR_DIR . '/class-widget-init.php';
		require_once SALA_ELEMENTOR_DIR . '/class-control-init.php';
	}

	function elementor_editor_scripts() {
		wp_enqueue_style( 'sala-elementor-editor', SALA_ELEMENTOR_ASSETS . '/css/editor.css' );
	}
	...
	public function add_cpt_support() {
		...
		if ( ! $cpt_support ) {
			...
			if ( ! in_array( 'sala_mega_menu', $cpt_support ) ) {
				$cpt_support[] = 'sala_mega_menu';
			}
	...
	public function admin_notice_missing_main_plugin() {
		...
		/* translators: 1: Plugin name 2: Elementor */
			esc_html__( '"%1$s" requires "%2$s" to be installed and activated.', 'sala' ),
			'<strong>' . esc_html__( 'Sala', 'sala' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'sala' ) . '</strong>'
		);
	...
	public function admin_notice_minimum_elementor_version() {
		...
		/* translators: 1: Plugin name 2: Elementor 3: Required Elementor version */
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'sala' ),
			'<strong>' . esc_html__( 'Sala', 'sala' ) . '</strong>',
			'<strong>' . esc_html__( 'Elementor', 'sala' ) . '</strong>',
	...
	public function admin_notice_minimum_php_version() {
		...
		/* translators: 1: Plugin name 2: PHP 3: Required PHP version */
			esc_html__( '"%1$s" requires "%2$s" version %3$s or greater.', 'sala' ),
			'<strong>' . esc_html__( 'Sala', 'sala' ) . '</strong>',
			'<strong>' . esc_html__( 'PHP', 'sala' ) . '</strong>',
