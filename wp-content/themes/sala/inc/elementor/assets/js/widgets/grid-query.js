	...
	function( $ ) {
		...
		$.fn.SalaGridQuery = function() {
			...
			function initFilterCount() {
				if ( ! $el.children( '.sala-grid-filter' ).data( 'filter-counter' ) ) {
					return;
				}
			...
			function handlerFilter() {
				$el.children( '.sala-grid-filter' ).on( 'click', '.btn-filter', function( e ) {
			...
			function handlerPagination() {
				...
				if ( $el.data( 'pagination' ) === 'load-more' ) {
					$el.children( '.sala-grid-pagination' )
					   .find( '.sala-load-more-button' )
					   .on( 'click', function( e ) {
						   e.preventDefault();
			...
			function handlerQuery( reset ) {
				...
				if ( infiniteLoader === true ) {
					loader = $el.find( '.sala-infinite-loader' );
				} else {
					loader = $grid.children( '.sala-grid-loader' );
				}
				...
				setTimeout( function() {
					...
					$.ajax( {
						...
						success: function( results ) {
							...
							$el.trigger( 'SalaQueryEnd', [ $el, $newItems ] );
			...
			function handlerQueryEnd() {
				...
				if ( foundPosts <= (
					...
					if ( $el.data( 'pagination' ) === 'load-more-alt' ) {
						...
						$el.children( '.sala-grid-pagination' ).hide();
					}

					$el.children( '.sala-grid-messages' ).show( 1 );
					setTimeout( function() {
						$el.children( '.sala-grid-messages' ).remove();
					}, 5000 );
				} else {
					if ( $el.data( 'pagination' ) === 'load-more-alt' ) {
						var loadMoreBtn = $( $el.data( 'pagination-custom-button-id' ) );
						...
						$el.children( '.sala-grid-pagination' ).show();
						$el.children( '.sala-grid-pagination' ).find( '.sala-load-more-button' ).show();
					}
