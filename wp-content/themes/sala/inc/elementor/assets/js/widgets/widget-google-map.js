(
	function( $ ) {
		...
		var GoogleMapHandler = function( $scope, $ ) {
			var $element = $scope.find( '.sala-google-map' );
				...
				$map.gmap3( settings )
				    ...
				    .on( {
					    mouseover: function( overlay ) {
						    ...
						    info.find( '.sala-map-overlay-content' ).show();
					    },
					    mouseout: function( overlay ) {
						    overlay.$.css( { zIndex: 1 } );

						    var info = overlay.$.find( '.gmap-info-wrapper' );
						    info.find( '.sala-map-overlay-content' ).hide();
					    }
				    } );
		...
		$( window ).on( 'elementor/frontend/init', function() {
			elementorFrontend.hooks.addAction( 'frontend/element_ready/sala-google-map.default', GoogleMapHandler );
		} );
	}
)( jQuery );
